...
class Hand_detector:
  ...
  def __init__(self):
    ...
    self.hand =self.handsModule.Hands(min_detection_confidence=0.6, min_tracking_confidence=0.6, max_num_hands=2) 
    self.ret=-1
    self.pom_thread=0
    self.memory=0
    # self.thread =None
  ...
  def detect(self):
    ...
    results = self.hand.process(self.img)
    ...
    if results.multi_hand_landmarks != None:
        if(length_hint(results.multi_hand_landmarks)==2):
                ...
                if((((results.multi_hand_landmarks[0].landmark[4].x - results.multi_hand_landmarks[1].landmark[4].x)**2 + (results.multi_hand_landmarks[0].landmark[4].y - results.multi_hand_landmarks[1].landmark[4].y)**2)**0.5)<=reference_distance_thumb):
                  # print("1")
