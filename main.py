...
class VideoStream:
    ...
    def __init__(self, resolution=(640, 480), framerate=30):
        ...
        # zmienna zapisujaca czy kamera jest zatrzymana
        self.stopped = False

    def start(self):
        # Wystrtowanie wątku, który odczytuje klatki
...
parser.add_argument('--threshold', help='Minimum confidence threshold for displaying detected objects',
                    default=0.6)
parser.add_argument('--resolution', help='Cammera resolution.',
...
#Improtowanie bibliotek
# Jesli tflite_runtime jest zainstalowany to import interpreter from tflite_runtime, 
# inaczej import tensorflow
# Jesli korzystamy z coral TPU to importujemy bibiloteke
...
# Pobranie scieżki do obecnego miejsca pracy
CWD_PATH = os.getcwd()

# Scieżka do modelu
PATH_TO_CKPT = os.path.join(CWD_PATH, MODEL_NAME, GRAPH_NAME)

# Scieżka do pliku map
PATH_TO_LABELS = os.path.join(CWD_PATH, MODEL_NAME, LABELMAP_NAME)

# Załadownie label mapy
...
# Jesli korzystamy z  Edge TPU, używammy specialnego argumentu load_delegate
if use_TPU:
...
# Inicjalizacja liczenia klatek
frame_rate_calc = 1
freq = cv2.getTickFrequency()

# Inicjalizacja video streamu
...
while True:
    # Start timeru do liczenia kaltek
    t1 = cv2.getTickCount()
   
    # Odczytanie klatki
    frame1 = videostream.read()

    # Skopiowanie obrazu, zmienienie rozmiarów, oraz zmiana na RGB wyjściowe rozmiary to [1xHxWx3]
    frame = frame1.copy()
    frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    frame_resized = cv2.resize(frame_rgb, (width, height))
    input_data = np.expand_dims(frame_resized, axis=0)
    
   # Jeśli miedzy wykrywaniem celu nie było przestoju większego niż 5 sek 
    #lub śledzona osoba wykonała gest w celu przestania śledzenia
    if((time.time()-timer) < 5):
        
        if floating_model:
            input_data = (np.float32(input_data) - input_mean) / input_std

        # Wprowadzenie danych do interpretera 
        interpreter.set_tensor(input_details[0]['index'], input_data)
        interpreter.invoke()

         # Rezulataty detekcji
        boxes = interpreter.get_tensor(output_details[boxes_idx]['index'])[0] #Tablica ramek danego obiektu
        classes = interpreter.get_tensor(output_details[classes_idx]['index'])[0]#Tablica klasy danego
        scores = interpreter.get_tensor(output_details[scores_idx]['index'])[0] #Tablica pewności co do 
                                                                                #klasy danego obiektu

       # Przejście po wszystkich wynikach i wyrysowanie ich
        for i in range(len(scores)):
            if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0) 
                and (labels[int(classes[i])] =='person' or labels[int(classes[i])] == 'Person')):

                 # Odczytanie wspołrzędnych ramki oraz upewnienie się, że znajdują się wewnątrz obrazu
                ...
                cv2.rectangle(frame, (xmin, ymin), (xmax, ymax), (255, 255, 0), 2)
                 ...
                 # Rysowanie nazwy klasy
                object_name = labels[int(classes[i])]   # Znalezienie klasy obiektu
                label = '%s: %d%%' % (object_name, int(scores[i] * 100))  
                labelSize, baseLine = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2) 
                label_ymin = max(ymin, labelSize[1] + 10)  #Offset do napisu nad ramką
                cv2.rectangle(frame, (xmin, label_ymin-labelSize[1] - 10), (xmin+labelSize[0], label_ymin+baseLine-10), (255, 255, 255), cv2.FILLED)  # Draw white box to put label text in
                cv2.putText(frame, label, (xmin, label_ymin - 7), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2) 
                 # Rysowanie okręgu na środku ramki
